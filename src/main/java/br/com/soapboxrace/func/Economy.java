...
public class Economy {
	...
	public Economy(String catalogName, String productId, Boolean type2) {
		try {
			if (catalogName == null || productId == null) {
				fx.log("  || -> Support for this product has not been added, yet.");
				return;
			}
			fx.log("  || -> Economy Module running.");
			if (type2) {
				...
				fx.log("  || -> Purchasing package " + productTitle + " for " + String.valueOf(amount) + (type == IGC ? " IGC" : " Boost") + ".");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void AddCommerce(String catalogName, String productId) {
		try {
			...
			String productTitle = catalog.getElementsByTagName("ProductTitle").item(catID).getTextContent().isEmpty() ? "Unknown Item"
					: catalog.getElementsByTagName("ProductTitle").item(catID).getTextContent();
			fx.log("    || -> Added package " + productTitle + " for " + String.valueOf(amount) + (type == IGC ? " IGC" : " Boost") + " to the shopping list.");
	...
	public boolean DoCommerce() {
		if (totalPrice[0] > 0) {
			fx.log("  || -> Trying for " + String.valueOf(totalPrice[0]) + " IGC.");
			type = IGC;
			amount = totalPrice[0];
		...
		if (totalPrice[1] > 0) {
			fx.log("  || -> Trying for " + String.valueOf(totalPrice[1]) + " Boost.");
			type = Boost;
			amount = totalPrice[1];
			if (!transCurrency(true))
				return false;
